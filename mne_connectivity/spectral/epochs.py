...
def _compute_freqs(n_times, sfreq, cwt_freqs, mode):
    ...
    if mode in ("multitaper", "fourier"):
        ...
        if cwt_freqs is None:
            raise ValueError(
                "define frequencies of interest using " "cwt_freqs"
            )
        else:
            cwt_freqs = cwt_freqs.astype(np.float64)
        if any(cwt_freqs > (sfreq / 2.0)):
            raise ValueError(
                "entries in cwt_freqs cannot be "
...
def _assemble_spectral_params(
    ...
    if mode == "multitaper":
        ...
        spectral_params.update(
            wavelets=morlet(
                sfreq, freqs, n_cycles=cwt_n_cycles, zero_mean=True
            )
        )
...
def _epoch_spectral_connectivity(
    ...
    for this_data in data:
        if mode in ("multitaper", "fourier"):
            if isinstance(this_data, _BaseSourceEstimate):
                _mt_spectra_partial = partial(
                    _mt_spectra, dpss=window_fun, sfreq=sfreq
                )
            ...
            if isinstance(this_data, _BaseSourceEstimate):
                cwt_partial = partial(
                    cwt, Ws=wavelets, use_fft=True, mode="same"
                )
                this_x_t = this_data.transform_data(
                    cwt_partial,
                    idx=sig_idx,
    ...
    if mode in ["multitaper", "fourier"]:
        ...
        for i_block, i in enumerate(range(0, n_cons, block_size)):
            ...
            csd = (
                x_t[idx_map[0][con_idx]] * x_t[idx_map[1][con_idx]].conjugate()
            )
    ...
    for this_data in data:
        ...
        if n_times is not None:
            if this_n_times != n_times:
                raise ValueError(
                    "all input time series must have the same "
...
def _check_estimators(method, mode):
    ...
    for this_method in method:
        if this_method in _CON_METHOD_MAP:
            ...
            raise ValueError(
                "%s is not a valid connectivity method" % this_method
            )
        else:
    ...
    n_comp_args = [
        len(inspect.signature(mtype.compute_con).parameters)
        for mtype in con_method_types
    ]
...
def spectral_connectivity_epochs(
    ...
    **Supported Connectivity Measures**
    ...
    (
        ...
        accumulate_psd,
        n_comp_args,
    ) = _check_estimators(method=method, mode=mode)
    ...
    for epoch_block in _get_n_epochs(data, n_jobs):
        ...
        if n_jobs == 1:
            ...
            for this_epoch in epoch_block:
                logger.info(
                    "    computing connectivity for epoch %d" % (epoch_idx + 1)
                )
    ...
    if indices is None:
        ...
        for this_con_flat in con_flat:
            ...
            this_con = this_con.reshape(
                (n_signals**2,) + this_con_flat.shape[1:]
            )
            con.append(this_con)
